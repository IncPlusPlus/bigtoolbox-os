...
public class Windows_10 extends Windows {
  ...
  private Windows_10(String minorVersion, String buildVersion) {
    ...
    try {
      ...
      try {
        throw new Exception(
            "minorVersion for Windows_10 must be an integer!\nmajorVersion for Windows_10 must be an double!");
      } catch (Exception e1) {
        e1.printStackTrace();
      }
    }
  ...
  private void grabVersionNumber() {
    {
      ...
      try {
        Process p =
            Runtime.getRuntime()
                .exec(
                    "cmd /c powershell -command \"(Get-ItemProperty 'HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion').ReleaseId\"");
        p.waitFor();
  ...
  private void grabBuildNumber() {
    ...
    StringBuilder buildBuilder =
        new StringBuilder(splitBuildText[2].length() + splitBuildText[3].length());
